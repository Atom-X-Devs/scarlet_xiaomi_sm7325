...
#ifdef CONFIG_MACH_XIAOMI
static int read_ss_auth_prop_id(struct battery_chg_dev *bcdev,
				struct psy_state *pst, u32 prop_id)
{
...
static int read_verify_digest_prop_id(struct battery_chg_dev *bcdev,
				      struct psy_state *pst, u32 prop_id)
{
...
static void StringToHex(char *str, unsigned char *out, unsigned int *outlen)
{
	char *p = str, high = 0, low = 0;
...
static struct quick_charge adapter_cap[11] = {
				      ...
				      struct class_attribute *attr, char *buf)
{
	...
	if (usb_present) {
		...
		if (real_charger_type == POWER_SUPPLY_USB_TYPE_PD_PPS) {
			if (verify_digiest == 1)
				result = (power_max >= 50) ? QUICK_CHARGE_SUPER :
							QUICK_CHARGE_TURBE;
			else
		...
		union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT_MAX:
		...
		break;
#ifdef CONFIG_MACH_XIAOMI
	/* ghostrider-reborn: workaround for charge counter */
			      ...
			      char *buf)
{
	...
	/*
	 * Sanity check to avoid real_type value above maximum
	 * 13(USB_FLOAT) which causes kernel crash
	 */
				  ...
				  char *buf)
{
	...
	rc = read_verify_digest_prop_id(bcdev, pst, XM_PROP_VERIFY_DIGEST);
	if (rc < 0)
		return rc;
				    ...
				    struct class_attribute *attr, char *buf)
{
	...
	case USBPD_UVDM_AUTHENTICATION:
		prop_id = XM_PROP_VDM_CMD_AUTHENTICATION;
		rc = read_ss_auth_prop_id(bcdev, pst, prop_id);
		if (rc < 0)
			return rc;
...
static ssize_t fg1_seal_set_store(struct class *c, struct class_attribute *attr,
				  const char *buf, size_t count)
{
	struct battery_chg_dev *bcdev =
		container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t fg1_seal_set_show(struct class *c, struct class_attribute *attr,
				 char *buf)
{
	struct battery_chg_dev *bcdev =
		container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t fg1_temp_max_show(struct class *c, struct class_attribute *attr,
				 char *buf)
{
	struct battery_chg_dev *bcdev =
		container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t fg1_temp_min_show(struct class *c, struct class_attribute *attr,
				 char *buf)
{
	struct battery_chg_dev *bcdev =
		container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t fg1_time_ht_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev =
		container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t fg1_time_ot_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev =
		container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t fg1_time_ut_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev =
		container_of(c, struct battery_chg_dev, battery_class);
...
static ssize_t fg1_time_lt_show(struct class *c, struct class_attribute *attr,
				char *buf)
{
	struct battery_chg_dev *bcdev =
		container_of(c, struct battery_chg_dev, battery_class);
...
{
	struct battery_chg_dev *bcdev =
		container_of(work, struct battery_chg_dev, notify_blankstate_work);
	int rc;
...
{
	struct battery_chg_dev *bcdev =
		container_of(work, struct battery_chg_dev, xm_prop_change_work.work);
