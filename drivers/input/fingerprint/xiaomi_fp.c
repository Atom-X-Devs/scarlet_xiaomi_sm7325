...
static inline ssize_t get_fingerdown_event(struct device *dev,
...
static inline ssize_t set_fingerdown_event(struct device *dev,
					   struct device_attribute *attr,
...
static inline ssize_t get_actpower_event(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct xiaomi_fp_data *xiaomi_fp = dev_get_drvdata(dev);
...
static inline ssize_t set_actpower_event(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	struct xiaomi_fp_data *xiaomi_fp = dev_get_drvdata(dev);
...
static inline int xiaomi_fp_probe(struct platform_device *pdev)
{
	...
	if (sysfs_create_group(&dev->kobj, &xm_fp_grp))
		dev_err(dev, "xiaomi_fp could not create sysfs\n");

	return 0;
}

static inline int xiaomi_fp_remove(struct platform_device *pdev)
{
