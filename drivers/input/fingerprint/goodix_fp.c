...
static inline void gf_setup(struct gf_dev *gf_dev)
{
	...
	gpio_direction_output(gf_dev->rst_gpio, 0);
...
static inline int enable_regulator_3V3(struct device *dev)
{
	int rc = 0;
	...
	if (regulator_set_load(p_3v3_vreg, 200000))
		goto exit;

	if (regulator_enable(p_3v3_vreg))
		goto exit;
...
static inline int disable_regulator_3V3(void)
{
	mutex_lock(&regulator_ocp_lock);
	if (!p_3v3_vreg)
		goto exit;
	...
	if (regulator_is_enabled(p_3v3_vreg))
		if (regulator_disable(p_3v3_vreg))
			goto exit;
			    ...
			    unsigned long arg)
{
	...
	if (!gf_dev->device_available && (cmd != GF_IOC_ENABLE_POWER ||
