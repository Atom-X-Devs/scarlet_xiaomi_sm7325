...
struct ir_spi_data {
	...
	int power_gpio, buffer_size;
	u8 *buffer;
	u16 nusers;
	dev_t devt;
};

static inline ssize_t ir_spi_chardev_write(struct file *file,
...
static inline int ir_spi_chardev_open(struct inode *inode, struct file *file)
{
...
static inline int ir_spi_chardev_close(struct inode *inode, struct file *file)
{
	struct ir_spi_data *idata = ir_spi_data_g;
...
static inline long ir_spi_chardev_ioctl(struct file *file, unsigned int cmd,
					unsigned long arg)
{
	struct ir_spi_data *idata = file->private_data;
	__u32 p;
